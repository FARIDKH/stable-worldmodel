...
class WireHarnessBasicEnv(gym.Env):
    ...
    def __init__(
        ...
        self._goal_image = None  # swm: visual goal, rendered at reset / target switch
        self._eval_goal  = None  # swm: dataset-eval override target (_set_goal_state)
        ...
        self.variation_space = swm_spaces.Dict({
            "mover": swm_spaces.Dict({
                ...
                "target_positions": swm_spaces.Box(
                    low=np.tile(lo, (n, 1)), high=np.tile(hi, (n, 1)),
                    shape=(n, 2), dtype=np.float32,
                    init_value=np.array(self.targets, dtype=np.float32),
                ),
            }),
        })
        ...
        if current_dist < self.goal_radius:
            ...
            if self._eval_goal is not None:
                ...
                if self.current_target_idx >= len(self.targets):
                    ...
                    self.mover.set_target(*self.targets[self.current_target_idx])
    ...
    def _teleport(self, position):
        ...
        self.data.joint(self.mover.joint_x).qpos[0] = px - self.mover.mu_start[0]
        self.data.joint(self.mover.joint_y).qpos[0] = py - self.mover.mu_start[1]
